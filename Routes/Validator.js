...
Validator.Tags = {
   ...
   oldPwdMismatch: "oldPwdMismatch", // Old password provided is incorrects
...
Validator.prototype.check = function(test, tag, params, cb) {
   ...
   if (this.errors.length) {
      if (this.res) {
         ...
         } else if (this.errors[0].tag === Validator.Tags.forbiddenField) {
            this.res.status(400).json([this.errors[0]]);
         } else
            this.res.status(400).json(this.errors);
         this.res = null;   // Preclude repeated closings
      }
      if (cb)
         cb(this); //this is the entire validator object
   }
...
Validator.prototype.checkPrsOK = function(prsId, cb) {
   var ssnPrsId = this.session.prsId;

   return this.check(this.session && 
    (this.session.isAdmin() || ssnPrsId === parseInt(prsId)), 
    Validator.Tags.noPermission, null, cb);
};
...
// Check presence of falsey property in |obj| for all fields in fieldList
Validator.prototype.checkForbidden = function(obj, fieldList, cb) {
   var self = this;
