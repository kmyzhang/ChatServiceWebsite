...
app.use((req, res, next) => {console.log('Before: ', req.body); next()});
app.use(bodyParser.json());
app.use((req, res, next) => {console.log('After: ', req.body); next()});
...
// Let's try a registration.  5-10 bugs in this one.
// alter user 'cstaley'@'%' identified with mysql_native_password by 'CASpw';
// alter user 'cstaley'@'%' identified with caching_sha2_password by 'CASpw';
app.post('/Prss', function(req, res) {
   ...
   if (req.body.role < 0 || req.body.role > 1)
      errorList.push({tag: "badValue", param: ["role"]});
